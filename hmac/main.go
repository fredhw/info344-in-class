...
//base64 converts bytes into text easily readable as human
...
func main() {
	...
	command := strings.ToLower(os.Args[1]) //position 0 will be the executable
	signingKey := os.Args[2]

	switch command {
	case "sign":
		sig, err := sign(signingKey, os.Stdin) // where you can read data from
		if err != nil {
